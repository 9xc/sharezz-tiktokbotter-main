try:
    ...
    input("Install requirements.txt")
...
class tiktok:
    ...
    def update_title(self):
        ctypes.windll.kernel32.SetConsoleTitleW(f"Sharezz | sent: {self.shared} | error: {self.errors} | https://github.com/9xc")
    ...
    def main(self):
        os.system(clear)
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW(f"Sharezz | https://github.com/9xc")
        print(Fore.LIGHTRED_EX + ascii_text)
        tiktok_url = str(input(f"       {Fore.WHITE}[{Fore.LIGHTBLUE_EX}Sharezz{Fore.WHITE}] URL: "))
        threads = int(input(f"       {Fore.WHITE}[{Fore.LIGHTBLUE_EX}Sharezz{Fore.WHITE}] THREADS: "))
        video_id = self.parse_url(tiktok_url)
        print(f"\n       {Fore.WHITE}[{Fore.LIGHTBLUE_EX}Sharezz{Fore.WHITE}] Sending shares...")
        print()
        def thread_starter():
